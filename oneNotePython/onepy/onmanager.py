...
class ONProcess():
    ...
    def create_new_page (self, section_id, new_page_style=0):
        ...
        """
        try:
            return(self.process.CreateNewPage(section_id, "", new_page_style))
        except Exception as e: 
            print("Unable to create the page")
            
    def close_notebook(self, notebook_id):
        try:
            return(self.process.CloseNotebook(notebook_id))
        except Exception as e: 
    ...
    def update_page_content(self, page_changes_xml_in, excpect_last_modified=0):
        try:
            return(self.process.UpdatePageContent(page_changes_xml_in, excpect_last_modified))
        except Exception as e: 
            raise e
